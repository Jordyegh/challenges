...
function first(){
  for(key in battlefield){
    if(count < 5){
      minion = document.getElementsByClassName("minions")[0];
      get = battlefield[key];
      minion.innerHTML = minion.innerHTML + "<div class='card "+key+"'>Name: "+get["name"]+"<br/>"+get["attack"]+" "+get["brute"]+" "+get["health"]+"<br/>"+get["race"]+"</div>";
    }
    else{
      minion = document.getElementsByClassName("minions")[1];
      get = battlefield[key];
      minion.innerHTML = minion.innerHTML + "<div class='card "+key+"'>Name: "+get["name"]+"<br/>"+get["attack"]+" "+get["brute"]+" "+get["health"]+"<br/>"+get["race"]+"</div>";
    }
  ...
  playCard("goblinranger");
}

function playCard(card){
  desc = battlefield[card]["text"].toLowerCase();
  ...
  for(i = 0; i < desc.length; i++){
    ...
    else if(ignore && desc.charAt(i) == ">"){
      ignore = 0;

      if(desc.charAt(i-1) == "p"){
        out+=" ";
      }
    }
  }
  ...
  split = out.split(" ");
  console.log("Playing card "+card+" with description of "+out);
  console.log("Description to array: "+split);
  console.log("========== OUTPUT: ==========");

  function runCondition(condition){
    for(i = 0; i < split.length; i++){
      if(split[i] == "deal"){
        if(Number(split[i+1]) > 0 && split[i+2] == "damage" && split[i+3] == "to"){
          if(split[i+4] == "an" && split[i+5] == "enemy"){
            if(split[i+6] == "minion"){
              console.log("Deal "+Number(split[i+1])+" damage to an enemy minion");
            }
            else{
              console.log("Deal "+Number(split[i+1])+" damage to an enemy");
            }
          ...
          else if(split[i+4] == "a" && split[i+5] == "minion"){
            console.log("Select any minion and deal "+Number(split[i+1])+" damage");
        ...
        for(u = 0; u < i; u++){
          if(split[u] == "enemy"){
            if(split[u+1] == "minion"){
              console.log("If an enemy minion is damaged");
            }
            else{
              console.log("If an enemy is damaged");
            }
          }
        }
      }
  ...
  for(i = 0; i < split.length; i++){
    if(split[i] == "on" && split[i+1] == "play:"){
      console.log("On Play:");
      runCondition("play");
    }
    else if(split[i] == "passive:"){
      console.log("Passive:");
      runCondition("passive");
    }
    else if(split[i] == "on" && split[i+1] == "death:"){
      console.log("On death:");
      runCondition("death");
    }
    else if(split[i] == "stealth"){
      console.log("Stealth");
    }
    else if(split[i] == "charge"){
      console.log("Charge");
    }
    else if(split[i] == "taunt"){
      console.log("Taunt");
    }
  }
}
